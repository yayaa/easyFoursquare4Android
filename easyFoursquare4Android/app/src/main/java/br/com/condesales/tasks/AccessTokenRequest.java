        ...
        if (code != null) {
            try {
                ...
                JSONObject tokenJson = executeHttpGet("https://foursquare.com/oauth2/access_token"
                        + "?client_id="
                        + FoursquareConfig.getClientId()
                        + "&client_secret="
                        + FoursquareConfig.getClientSecret()
                ...
                SharedPreferences settings = mActivity.getSharedPreferences(FoursquareConfig.SHARED_PREF_FILE, 0);
                SharedPreferences.Editor editor = settings.edit();
                editor.putString(FoursquareConfig.ACCESS_TOKEN, token);
                // Commit the edits!
                editor.commit();
